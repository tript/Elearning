  ...
  def lessons
    if params[:school_id]
      ...
      if @school.grade_id == 7
        ...
        @lessons = Lesson.joins(user: :school).where("schools.pdt_id = ? OR schools.id = ?", @school.id, @school.id).where(class_id: params[:class_id], subject_id: params[:subject_id], type_id: params[:type_id]).paginate(page: params[:page], per_page: 30)
      end
    else
      @lessons = Lesson.where(class_id: params[:class_id], subject_id: params[:subject_id], type_id: params[:type_id]).paginate(page: params[:page], per_page: 30)
    end
